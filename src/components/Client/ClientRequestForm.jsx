...
const levelLabel = (v) => (
{ beginner:'Débutant', junior:'Junior', intermediate:'Intermédiaire', senior:'Senior', expert:'Expert' }
...
const [weights, setWeights] = useState({ skills: 5, tjm: 3, telework: 2, availability: 2 })
...
async function handleComputeShortlist() {
 ...
 const derivedWeights = {
   ...
   availability: 0 // on ne compte plus la dispo
 };
...
<div>
  ...
  const expectedTjm = (kind === 'expertise')
    ? '—'
    : (String(tjmMin).trim() !== '' || String(tjmMax).trim() !== '')
      ? `${String(tjmMin).trim() || '…'}–${String(tjmMax).trim() || '…'} €`
      : '—'

  const expectedTT = (kind === 'expertise')
    ? '—'
    : (remote ? `${remoteDaysCount || 0} j/s` : (city?.name || '—'))

  // --- Données profil (résultat) ---
  const perTech = getPerTech(r)
  const perTechMap = new Map(perTech.map(d => [String(d.techName || '').toLowerCase(), d]))
  const profileTjm = (r.details?.tjmMin && r.details?.tjmMax)
    ? `${r.details.tjmMin}–${r.details.tjmMax} €`
    : (r.details?.tjmValue ? `${r.details.tjmValue} €` : '—')
  const profileTT = r.details?.teleworkDays ? `${r.details.teleworkDays} j/s` : '0 j/s'

  // --- Importances (côté client) ---
  const tjmImportance = (kind === 'expertise') ? 0 : (Number(tjmWeight) || 0)
  const locImportance = (kind === 'expertise') ? 0 : (Number(locationWeight) || 0)
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50 text-gray-700">
            <tr>
              <th className="px-3 py-2 text-left font-semibold w-40">Critère</th>
              <th className="px-3 py-2 text-left font-semibold">Attendu</th>
              <th className="px-3 py-2 text-left font-semibold">Profil</th>
              <th className="px-3 py-2 text-left font-semibold w-28">Importance</th>
              <th className="px-3 py-2 text-right font-semibold w-28">Match</th>
            </tr>
          </thead>
          <tbody>
            {reqTechList.length > 0 ? (
              reqTechList.map((t, idx) => {
                const found = perTechMap.get(t.name.toLowerCase())
                const profileLevel = found?.profileLevel ? levelLabel(found.profileLevel) : '—'
                const match = Math.max(0, Math.min(100, found?.match ?? 0))
                const critCell = idx === 0 ? (
                  <td className="px-3 py-2 font-semibold align-top" rowSpan={reqTechList.length}>
                    Technologies
                  </td>
                ) : null
                return (
                  <tr key={`${t.name}-${idx}`} className={idx === 0 ? 'border-t' : ''}>
                    {critCell}
                    <td className="px-3 py-2">{t.name} ({t.level})</td>
                    <td className="px-3 py-2">{t.name} ({profileLevel})</td>
                    <td className="px-3 py-2 tabular-nums">{t.weight}</td>
                    <td className="px-3 py-2 text-right tabular-nums">{match}%</td>
                  </tr>
                )
              })
            ) : (
              <tr className="border-t">
                <td className="px-3 py-2 font-semibold">Technologies</td>
                <td className="px-3 py-2">—</td>
                <td className="px-3 py-2">—</td>
                <td className="px-3 py-2 tabular-nums">0</td>
                <td className="px-3 py-2 text-right tabular-nums">{getSkillsTotal(r) ?? 0}%</td>
              </tr>
            )}

            <tr className="border-t">
              <td className="px-3 py-2 font-semibold">TJM</td>
              <td className="px-3 py-2">{expectedTjm}</td>
              <td className="px-3 py-2">{profileTjm}</td>
              <td className="px-3 py-2 tabular-nums">{tjmImportance}</td>
              <td className="px-3 py-2 text-right tabular-nums">{r.details?.tjm ?? 0}%</td>
            </tr>

            <tr className="border-t">
              <td className="px-3 py-2 font-semibold">Télétravail</td>
              <td className="px-3 py-2">{expectedTT}</td>
              <td className="px-3 py-2">{profileTT}</td>
              <td className="px-3 py-2 tabular-nums">{locImportance}</td>
              <td className="px-3 py-2 text-right tabular-nums">{r.details?.telework ?? 0}%</td>
            </tr>
          </tbody>
        </table>
      </div>

      <ShortlistDebug result={r} weights={lastWeights} reqTechs={techRows} />
    </>
  )
})()}
                    </div>
